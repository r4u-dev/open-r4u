...
from app.services.task_grouping import TaskGrouper, find_or_create_task_for_trace, group_all_traces
...
async def test_extract_instructions_from_system_message(test_session: AsyncSession):
    ...
    query = select(Trace).where(Trace.id == trace.id).options(selectinload(Trace.input_items))
...
async def test_group_traces_by_path(test_session: AsyncSession):
    ...
    # Check that task has templated instructions
    assert task.instructions and "{var_" in task.instructions
...
async def test_find_matching_task(test_session):
    ...
    task = Task(
        project_id=project.id,
        path="/greet",
        prompt="You are a helpful assistant. Greet the user politely.",
        model="gpt-4",
        instructions="Greet user {var_0} politely.",
    )
...
async def test_api_group_all_traces(client: AsyncClient, test_session: AsyncSession):
    ...
    # Group 2: Weather queries
    for city in ["NYC", "LA", "Chicago"]:
        trace = Trace(
            project_id=project.id,
            model="gpt-4",
            path="/api/weather",
            instructions=f"Get weather for {city}",
    ...
    # Check that tasks have templated instructions
    for task_data in data:
        assert "{var_" in task_data["instructions"]
