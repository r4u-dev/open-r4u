...
async def test_no_auto_match_when_no_similar_task(
    ...
    # Create a task with different instructions
    task = Task(
        project_id=project.id,
        path="/weather",
...
async def test_auto_match_respects_similarity_threshold(
    ...
    # Create a task with specific instructions
    task = Task(
        project_id=project.id,
        path="/api",
        prompt="You are a helpful assistant.",
        model="gpt-4",
        instructions="Process the user's request carefully",
    )
    test_session.add(task)
    await test_session.commit()
...
async def test_auto_match_with_system_message_instructions(
    ...
    task = Task(
        ...
        prompt="You are a helpful assistant.",
        model="gpt-4",
        instructions="Help the user with {var_0} questions",
    )
    test_session.add(task)
    await test_session.commit()
...
async def test_batch_grouping_creates_tasks_for_ungrouped_traces(
    ...
    assert "{var_" in task["instructions"]  # Should have templated instructions
...
async def test_subsequent_traces_auto_match_after_batch_grouping(
    ...
    response = await client.post(
        ...
        json={
            ...
            "started_at": "2025-10-17T10:00:00Z",
            "input": [
                {"type": "message", "role": "user", "content": "Hello Charlie"}
            ],
        },
    )
