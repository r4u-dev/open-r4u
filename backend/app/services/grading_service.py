...
async def get_grader(session: AsyncSession, grader_id: int) -> Grader:
    """Get a grader by ID."""
...
async def update_grader(
    ...
    **updates: Any,
    ...
    """Update a grader."""
    grader = await get_grader(session, grader_id)
...
async def delete_grader(session: AsyncSession, grader_id: int) -> None:
...
def _render_grading_prompt(grader: Grader, context: str) -> str:
...
async def execute_grading(
    ...
    context = _prepare_target_context(trace, execution_result)
    
    started_at = datetime.now(timezone.utc)
    
    try:
        rendered_prompt = _render_grading_prompt(grader, context)
    ...
    if not execution_result_base.error:
        score_float, score_boolean, reasoning_text, confidence = _parse_grading_response(
...
async def get_grade(session: AsyncSession, grade_id: int) -> Grade:
    """Get a grade by ID."""
...
async def list_grades_for_trace(
    session: AsyncSession, trace_id: int
) -> list[Grade]:
...
async def list_grades_for_execution(
    session: AsyncSession, execution_result_id: int
) -> list[Grade]:
...
async def list_grades_for_grader(
    session: AsyncSession, grader_id: int
) -> list[Grade]:
...
async def delete_grade(session: AsyncSession, grade_id: int) -> None:
    """Delete a grade."""
