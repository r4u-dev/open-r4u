...
class TaskGrouper:
    ...
    async def find_or_create_task_for_trace(
        ...
        new_task = await self._create_task_from_similar_traces(trace, instructions, session)
    ...
    def _extract_instructions(self, trace: Trace) -> str:
        ...
        for input_item in sorted(trace.input_items, key=lambda x: x.position):
            if input_item.type.value == "message":
                ...
                if role in [MessageRole.SYSTEM.value, MessageRole.DEVELOPER.value] and content:
    ...
    async def _create_task_for_group(
        ...
        if len(instruction_strings) >= 2:
            ...
            templated_instructions = instruction_strings[0] if instruction_strings else reference_trace.prompt or ""
        
        # Determine common prompt template
        prompt_strings = [t.prompt for t in traces if t.prompt]
        if len(prompt_strings) >= 2:
            templated_prompt = infer_template_from_strings(prompt_strings[:self.max_sample_size])
        else:
        ...
        task = Task(
            ...
            instructions=templated_instructions,
