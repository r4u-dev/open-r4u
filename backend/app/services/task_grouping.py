...
class TaskGrouper:
    ...
    async def find_or_create_task_for_trace(
        ...
        matching_task = await self._find_matching_task(trace, instructions)
    ...
    async def group_all_traces(self) -> list[Task]:
        ...
        for path, path_traces in path_groups.items():
            ...
            for instruction_group in instruction_groups:
                if len(instruction_group) >= self.min_cluster_size:
                    task = await self._create_task_for_group(instruction_group)
                    if task:
    ...
    async def _create_task_from_similar_traces(
        ...
        task = await self._create_task_for_group(similar_traces)
        return task
    ...
    async def _create_task_for_group(
        ...
        # Get project name for task creation
        from app.models.projects import Project

        project_query = select(Project).where(Project.id == reference_trace.project_id)
        ...
        implementation_data = ImplementationCreate(
            ...
            max_output_tokens=reference_trace.total_tokens or 4096,
        )
        ...
        implementation = await self.implementation_service.get_implementation(
...
async def group_all_traces(
    ...
    return await grouper.group_all_traces()
