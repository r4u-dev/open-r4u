...
type SortField = 'status' | 'name' | 'cost' | 'latency' | 'quality' | 'lastActivity';
...
export function TasksTable({ tasks }: TasksTableProps) {
  ...
  const enhancedTasks: TaskWithMetrics[] = tasks.map(task => {
    ...
    const healthStatus = performanceScore > 80 ? 'healthy' : 
                        performanceScore > 60 ? 'moderate' : 'poor';
    
    const trend = Math.random() > 0.5 ? 'up' : Math.random() > 0.5 ? 'down' : 'stable';
    
    const lastExecutionStatus = Math.random() > 0.1 ? 'success' : 
                               Math.random() > 0.5 ? 'error' : 'never';
    
    const implementationType = Math.random() > 0.5 ? 'reasoning' : 
                              Math.random() > 0.5 ? 'functional' : 'workflow';
        ...
        <tbody>
          {sortedTasks.map((task) => (
            ...
            >
              ...
              <td className="px-4 py-3">
                <span className="text-sm">${task.p95Cost.toFixed(4)}</span>
              </td>
              <td className="px-4 py-3">
                <span className="text-sm">{Math.round(task.p95Latency * 1000)}ms</span>
              </td>
              <td className="px-4 py-3">
                <span className="text-sm">{task.p5Quality.toFixed(0)}%</span>
              </td>
              <td className="px-4 py-3">
