...
class EvaluationsApiService {
  ...
  async createEvaluation(data: CreateEvaluationRequest): Promise<ApiResponse<EvaluationSummary>> {
    return apiClient.post<EvaluationSummary>(this.baseEndpoint, data);
  }

  async getEvaluation(evaluationId: string): Promise<ApiResponse<EvaluationDetail>> {
    return apiClient.get<EvaluationDetail>(`${this.baseEndpoint}/${evaluationId}`);
  }
  ...
  async listEvaluationResults(evaluationId: string): Promise<ApiResponse<EvaluationResultItem[]>> {
    return apiClient.get<EvaluationResultItem[]>(`${this.baseEndpoint}/${evaluationId}/results`);
  }

  async deleteEvaluation(evaluationId: string): Promise<ApiResponse<void | null>> {
    return apiClient.delete<void | null>(`${this.baseEndpoint}/${evaluationId}`);
  }

  async getTaskEvaluationMetrics(taskId: string): Promise<ApiResponse<{
    ...
    return apiClient.get(`${this.baseEndpoint}/metrics?${query.toString()}`);
  }
}
