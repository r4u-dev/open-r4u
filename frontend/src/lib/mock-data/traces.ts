    ...
    for (let i = 1; i <= 150; i++) {
        ...
        const model =
            models[provider as keyof typeof models][
                Math.floor(
                    Math.random() *
                        models[provider as keyof typeof models].length,
                )
            ];
        const type = types[Math.floor(Math.random() * types.length)];
        const status = statuses[Math.floor(Math.random() * statuses.length)];
        const taskVersion =
            taskVersions[Math.floor(Math.random() * taskVersions.length)];
        const endpoint =
        ...
        const trace: Trace = {
            ...
            metrics: {
                ...
                completion_tokens: Math.floor(Math.random() * 300) + 30,
                total_tokens: Math.floor(Math.random() * 800) + 80,
            },
            outputItems: isError
                ? []
                : [
